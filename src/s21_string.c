...
s21_size_t s21_strcspn(const char *str1, const char *str2) {
    ...
    str1_len = (int)s21_strlen(str1);
    str2_len = (int)s21_strlen(str2);
...
void *s21_trim(const char *src, const char *trim_chars) {
    ...
    s21_memset(edited_src, 0, 300);

    if (s21_strlen(trim_chars) == 0) {
         s21_strcpy((char*)trim_chars, " \t\n\r\0\v");
    }

    for (; s21_strchr(trim_chars, src[str_st]) && src[str_st];) {
        str_st++;
    }

    for (; s21_strrchr(trim_chars, src[src_len]) && src_len >= str_st;) {
        str_end = src_len--;
    }

    sym_num = str_end - str_st;

    s21_strncat(edited_src, &src[str_st], sym_num);
    s21_strcat(edited_src, "\0");

    return edited_src;
}


void *s21_insert(const char *str, const char *insertion, s21_size_t index) {
     s21_size_t insertion_len = s21_strlen(insertion);
     s21_size_t str_len = s21_strlen(str);
     static char res[2000];
     s21_memset(res, '\0', 2000);
     char *pstr = (char*)str;

     if (index <= str_len) {
         s21_memmove(res, pstr, index);
         s21_memmove(&res[index + insertion_len], &pstr[index], str_len - index);
         s21_memmove(&res[index], insertion, insertion_len);
...
void *s21_to_upper(const char *str) {
    int i = 0, str_len = (int)s21_strlen(str);
    static char res[2000];
    s21_memset(res, '\0', 2000);
    char *pstr = (char*)str;
...
int sprintf_float(char *str, va_list *list, int str_index, int *number_precision, int flag_space,
    ...
    int align = width - (int)s21_strlen(int_float_number);
...
int sprintf_unsigned(char *str, va_list *list, int str_index, int number_precision, int flag_minus,
    ...
    if (number_precision > (int)s21_strlen(p)) {
        char temp[100];
        s21_memset(temp, '0', number_precision);
        s21_strcpy(&temp[number_precision - s21_strlen(p)], p);
        s21_strcpy(p, temp);
    }
    int align = width - s21_strlen(p);
...
int sprintf_string(char *str, va_list *list, int str_index, int number_precision,
    ...
    char string[100];
    s21_strcpy(string, string_ini);
    int written_chars_count = 0;

    if (flag_precision && number_precision < (int)s21_strlen(string)) {
        s21_memset(&string[number_precision], '\0', 1);
    }
    int align = width - (int)s21_strlen(string);
...
int sprintf_digit(char *str, va_list *list, int str_index, int number_precision, int flag_space,
    ...
    if (number_precision > (int)s21_strlen(p)) {
        char temp[100];
        s21_memset(&temp[0], '0', number_precision);
        s21_strcpy(&temp[number_precision - (int)s21_strlen(p)], p);
        s21_strcpy(p, temp);
    }
    int align = width - (int)s21_strlen(p);
...
int float_filling(char *int_float_number, char *int_part_arr, char *float_part_arr,
    ...
    s21_strcat(int_float_number, int_part_arr);

    if (number_precision) {
        s21_strcat(int_float_number, ".");
    }

    s21_strncat(int_float_number, float_part_arr, number_precision);
...
char *float_to_char(long double number, int number_precision) {
    ...
    long int over_last_digit = ((unsigned long long int)(number_cpy* s21_pow(10, number_precision+1)))%10;
...
int char_to_int(char* str) {
    ...
    for (int i = 0; i < temp; i++) {
         res += ((int)str[i] - '0') * s21_pow(10, str_len - 1);
         str_len--;
    }
...
long long int s21_pow(long long int number, long long int power) {
...
void s21_clean(char *str, char *src, s21_size_t size) {
    s21_memset(str, '\0', size);
    s21_memset(src, '\0', size);
}
